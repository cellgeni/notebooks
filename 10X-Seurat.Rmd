...
```{r}
pbmc <- FindVariableFeatures(object = pbmc, mean.function = ExpMean, dispersion.function = LogVMR, mean.cutoff = c(0.0125, 3), nfeatures = 2000)
```
...
```{r}
pbmc <- ScaleData(object = pbmc, vars.to.regress = c("nCount_RNA", "percent.mito"))
```
...
CellScatter(object = pbmc, cell1 = "AGTCTACTAGGGTG-1", cell2 = "CACAGATGGTTTCT-1")
```
...
pbmc <- FindClusters(object = pbmc, resolution = 0.8, verbose = FALSE)
```

```{r}
# Demonstration of how to plot two tSNE plots side by side, and how to color
# points based on different criteria
plot1 <- TSNEPlot(object = pbmc, label = TRUE)
plot2 <- TSNEPlot(object = pbmc, group.by = "ClusterNames_0.6", label = TRUE)
